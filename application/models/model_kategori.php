...
class Model_kategori extends CI_Model {
	...
	public function _get()
	{
		...
		if($search['value'] != ""){
			...
			$query .= "(m_categories_nama = '". $search['value'] ."')";
		}
        ...
        foreach ($data as $d) {
			...
			$r[0] = $d['m_categories_id'];
			$r[1] = $i;
			$r[2] = $d['m_categories_nama'];
            if ($d['m_categories_active'] == 'T') {
    ...
    public function _update()
    {
        ...
        $filter = array('m_categories_id'=>$id);
        $data = array('m_categories_nama'=>$nama);
        
        if ($id) {
            $desc = 'diubah';
            $this->db->where($filter);
            $this->model_public->update('m_categories',$data);
        }else{
            $desc = 'ditambah';
            $this->model_public->insert('m_categories', $data);
        }
    ...
    public function _delete()
    {
        ...
        $aktif = ($aktif == 'Y') ? 'T' : 'Y';
        $filter = array('m_categories_id'=>$id);
        $data = array('m_categories_active'=>$aktif);
...
/* End of file model_m_categories.php */
/* Location: ./application/models/model_m_categories.php */